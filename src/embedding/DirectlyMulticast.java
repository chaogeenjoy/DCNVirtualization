...
public class DirectlyMulticast {

	public boolean directMulticast(MulticastRequest request, Layer phyLayer){
		boolean flag=true;
		if(!this.enoughEdgeBandwidth(request, phyLayer)){
			...
			while(itr0.hasNext()){
				...
				Iterator<String> itr1=phyLayer.getLinklist().keySet().iterator();
				while(itr1.hasNext()){
					Link link=(Link)(phyLayer.getLinklist().get(itr1.next()));
